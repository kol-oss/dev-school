...
function equalSumIndex(listOfNumbers) {
    const sumOfNumbers = listOfNumbers.reduce((acc, value) => acc + value);
    let leftSum = 0;
    let rightSum = sumOfNumbers;
...
for (const exampleData of exampleCases) {
    console.log(`Result of input [${exampleData}]: ${equalSumIndex(exampleData)}`)
}